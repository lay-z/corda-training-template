...
class IOUIssueFlowTests {
    ...
    fun flowReturnsCorrectlyFormedPartiallySignedTransaction() {
        ...
        // No outputs, one output IOUState and a command with the right properties.
    ...
    /**
     * Task 5.
     * Check the vaults of both parties and make sure that there is an IOUstate that exists there?
     */
    @Test
    fun partyVaultsContainsCorrectDetailsOfIOUState() {
        val lender = a.info.chooseIdentityAndCert().party
        val borrower = b.info.chooseIdentityAndCert().party
        val iou = IOUState(10.POUNDS, lender, borrower)
        val flow = IOUIssueFlow(iou)
        ...
        val stx = future.getOrThrow()
        listOf(a, b).forEach {
            val states = it.services.vaultService.queryBy(IOUState::class.java)
            assertEquals(states.states.first().state.data, iou)
        }
    }
}
